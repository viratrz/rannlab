...
class block_cocoon_tstmnls_4 extends block_base {

    /**
     * Start block instance.
     */
    function init() {
        $this->title = get_string('pluginname', 'block_cocoon_tstmnls_4');
    }
    ...
    function get_content() {
        ...
        if ($data->slidesnumber > 0) {
            $text = '	<section id="our-testimonials" class="our-testimonial" data-ccn-c="color_bg" data-ccn-co="bg" data-ccn-cv="'.$this->content->color_bg.'">
		<div class="container">
			<div class="row">
			...
			<div class="row">
				<div class="col-lg-12">
					<div class="testimonial_slider_home2 home14 testimonial_slider_home2-'.$this->instance->id.'">';
          $fs = get_file_storage();
            for ($i = 1; $i <= $data->slidesnumber; $i++) {
                ...
                $files = $fs->get_area_files($this->context->id, 'block_cocoon_tstmnls_4', 'slides', $i, 'sortorder DESC, id ASC', false, 0, 0, 1);

                if (!empty($data->$sliderimage) && count($files) >= 1) {
                  $mainfile = reset($files);
                  $mainfile = $mainfile->get_filename();
                  $data->$sliderimage = moodle_url::make_file_url("$CFG->wwwroot/pluginfile.php","/{$this->context->id}/block_cocoon_tstmnls_4/slides/" . $i . '/' . $mainfile);
                } else {
...
';
    ...
    function instance_config_save($data, $nolongerused = false) {
        ...
        for($i = 1; $i <= $data->slidesnumber; $i++) {
            ...
            file_save_draft_area_files($data->$field, $this->context->id, 'block_cocoon_tstmnls_4', 'slides', $i, $filemanageroptions);
        }
    ...
    function instance_delete() {
        global $DB;
        $fs = get_file_storage();
        $fs->delete_area_files($this->context->id, 'block_cocoon_tstmnls_4');
        return true;
    }
    ...
    public function instance_copy($fromid) {
        ...
        for($i = 1; $i <= $data->slidesnumber; $i++) {
            ...
            if (!$fs->is_area_empty($fromcontext->id, 'block_cocoon_tstmnls_4', 'slides', $i, false)) {
                $draftitemid = 0;
                file_prepare_draft_area($draftitemid, $fromcontext->id, 'block_cocoon_tstmnls_4', 'slides', $i, $filemanageroptions);
                file_save_draft_area_files($draftitemid, $this->context->id, 'block_cocoon_tstmnls_4', 'slides', $i, $filemanageroptions);
            }
        }
