...
class block_cocoon_parallax_subscribe_2_edit_form extends block_edit_form{
    protected function specific_definition($mform){
      ...
      $mform->setDefault('config_form_title', 'Create Free Account To Get');
    ...
    function set_data($defaults){
        ...
        file_prepare_draft_area($draftitemid, $this->block->context->id, 'block_cocoon_parallax_subscribe_2', 'content', 0,
            array('subdirs' => true));
        $entry->attachments = $draftitemid;
        parent::set_data($defaults);
        if ($data = parent::get_data()) {
            file_save_draft_area_files($data->config_image, $this->block->context->id, 'block_cocoon_parallax_subscribe_2', 'content', 0,
                array('subdirs' => true));
        }
