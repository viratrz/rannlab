 ...
 */
    ...
    function add_activity_resources() {
        global $USER, $CFG, $DB, $OUTPUT;

        $activity_add = new stdClass;
        $activity_add->items = array();
        $activity_add->icons = array();
        $activity_add->footer = '';
        ...
        $courserenderer = $format->get_renderer($page);
        ...
        if (!$isediting) {
            ...
            if (!empty($modinfo->sections[0])) {
                foreach($modinfo->sections[0] as $cmid) {
                    ...
                    $activity_add->items[] = html_writer::div($content, 'main-menu-content section');
                }
            }
            return $activity_add;
        }
        ...
        if ($ismoving) {
            $activity_add->icons[] = $OUTPUT->pix_icon('t/move', get_string('move'));
            $activity_add->items[] = $USER->activitycopyname.'&nbsp;(<a href="'.$CFG->wwwroot.'/course/mod.php?cancelcopy=true&amp;sesskey='.sesskey().'">'.$strcancel.'</a>)';
        }
            ...
            foreach ($modinfo->sections[0] as $modnumber) {
                ...
                if ($mod->visible || has_capability('moodle/course:viewhiddenactivities', $mod->context)) {
                    if ($ismoving) {
                        ...
                        $activity_add->items[] = html_writer::link($movingurl, '', array('title' => $strmovefull,
                            'class' => 'movehere'));
                       $activity_add->icons[] = '';
                    }
                    ...
                    $activity_add->items[] = html_writer::div($content, 'main-menu-content');
                }
            }
        }
        ...
        if ($ismoving) {
            ...
            $activity_add->items[] = html_writer::link($movingurl, '', array('title' => $strmovefull, 'class' => 'movehere'));
            $activity_add->icons[] = '';
        }

        if ($course->id === SITEID) {
            $activity_add->footer = $courserenderer->course_section_add_cm_control($course,
