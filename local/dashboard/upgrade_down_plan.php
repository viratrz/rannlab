...
if (isset($_GET['id']) && isset($_GET['uni_id'])) 
{
    ...
    $new_package = $DB->get_record_sql("SELECT * FROM {package} WHERE id=$pac_id");

    if ($my_package->package_value < $new_package->package_value) {
        $message = "Your plan is upgrade successfully!";
    } else {
        $message = "Your plan is down successfully!";
    }
    ...
    if ($status) {
        ...
        redirect("subcription_payment.php?un_id=$uni_id", "$message",null, \core\output\notification::NOTIFY_SUCCESS);
    }
    else {
