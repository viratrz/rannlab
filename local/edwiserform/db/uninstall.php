...
function xmldb_local_edwiserform_uninstall() {
    global $DB, $CFG;
    $fs = get_file_storage();
    $fs->delete_area_files(context_system::instance()->id, EDWISERFORM_COMPONENT, EDWISERFORM_USER_FILEAREA);
    $fs->delete_area_files(context_system::instance()->id, EDWISERFORM_COMPONENT, EDWISERFORM_SUCCESS_FILEAREA);
    $fs->delete_area_files(context_system::instance()->id, EDWISERFORM_COMPONENT, EDWISERFORM_EMAIL_FILEAREA);
    return true;
}
