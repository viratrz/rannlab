...
import {match, unique, uuid, getString, closest, closestFtype} from '../common/utils';
...
const defaultElements = [{
  ...
  attrs: {
    className: '',
    href: M.cfg.wwwroot + '/login/index.php',
    style: ''
  },
  config: {
  ...
  content: getString('login')
}, {
  tag: 'a',
  attrs: {
    className: '',
    href: M.cfg.wwwroot + '/login/signup.php',
    style: ''
  },
  config: {
    label: getString('register'),
    editable: true,
    hideLabel: true
  },
  meta: {
...
/**
  * Return row id of column
  * @param  {String} columnID
  * @param  {Boolean} next if true then return nextSibling if present
  * @param  {Boolean} previous if true then return previousSibling if present
  * @return {Object} rowID and index column in row
  */
function getColumn(columnID, next = true, previous = true) {
  ...
  } else if (column.previousSibling && column.previousSibling.classList.contains('stage-columns')) {
    return {
      column: column.previousSibling,
      dir: -1
    };
  }
  const row = closestFtype(column);
  column = new Column();
  row.append(column);
...
/**
  * Adding composite elements in builder
  * @param {String} columnID
  * @param  {Object} field
  * @return {DOM} fields
  */
function addCompositeFields(columnID, field) {
  const type = field.meta.id;
  const compositeFields = {
    name: () => {
      const column = getColumn(columnID, true, true);
      const firstname = new Field('text-input', null, {
        attrs: {
          name: 'firstname'
        },
        config: {
      ...
      const lastname = new Field('text-input', null, {
        attrs: {
          name: 'lastname'
        },
        config: {
      ...
      column.column.appendChild(addField);
      dom.emptyClass(column.column);
      data.saveFieldOrder(column.column);
      dom.emptyClass(column);
      return returnField;
    },
    address: () => {
      const column = getColumn(columnID, true, true);
      const city = new Field('text-input', null, {
        attrs: {
          name: 'city'
        },
        config: {
      ...
      const country = new Field('country', null, {
        attrs: {
          name: 'country'
        },
        config: {
  ...
  if (h.get(compositeFields, type)) {
    return compositeFields[type]();
  }
...
class Controls {
  ...
  constructor(controlOptions, formID) {
    ...
    this.controlEvents = {
      ...
      //   },
      click: evt =>_this.addElement(evt.target)
      // Mousedown: evt => {
    ...
    const controlsToggle = {
      ...
      action: {
        click: event => {
          ...
          document.querySelector('#efb-cont-form-preview .efb-form-style-container').parentElement.classList.toggle('show-styles');
   ...
   * @param {DOM} element from controls
   */
  addElement(element) {
    if (dom.license != 'available') {
      dom.proWarning(element.textContent);
      return;
    }
    while (!element.classList.contains('field-control')) {
      element = element.parentElement;
    }
    const id = element.id;
    const row = dom.addRow();
    if (!element.classList.contains('layout-tab-control')) {
