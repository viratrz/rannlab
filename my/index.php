    ...
    <!--<div class="row">
       ...
       $coursemodulecount = $DB->count_records("course_modules", ['tenent_id'=>$university_id]);
        ...
        if (is_siteadmin()) {
            ...
            $unassigned_count_sql = "SELECT COUNT(bht.id) FROM {block_helpdesk_ticket} bht 
            JOIN {block_helpdesk_hd_user} bhhu ON bhhu.id = bht.hd_userid 
            JOIN {university_user} uu ON uu.userid = bhhu.userid
            JOIN {user} u ON u.id = uu.userid
            WHERE assigned_refs = ? AND uu.university_id = ?";
            $unassignedticket = $DB->count_records_sql($unassigned_count_sql, [0,$SESSION->university_id]);
            // $unassignedticket = $DB->count_records("block_helpdesk_ticket",['assigned_refs'=>'0']);
            $openticket = $DB->get_records_sql("SELECT bht.* FROM {block_helpdesk_ticket} bht
                JOIN {block_helpdesk_hd_user} bhhu ON bhhu.id = bht.hd_userid 
                JOIN {university_user} uu ON uu.userid = bhhu.userid
                JOIN {user} u ON u.id = uu.userid
                where bht.status = ? AND uu.university_id = ?",[1,$SESSION->university_id]);
            $unresolvedticket_count_sql = "SELECT COUNT(bht.id) FROM {block_helpdesk_ticket} bht 
            JOIN {block_helpdesk_hd_user} bhhu ON bhhu.id = bht.hd_userid 
            JOIN {university_user} uu ON uu.userid = bhhu.userid
            JOIN {user} u ON u.id = uu.userid
            WHERE status = ? AND uu.university_id = ?";
            $unresolvedticket = $DB->count_records_sql($unresolvedticket_count_sql, [2, $SESSION->university_id]);
            // $unresolvedticket = $DB->count_records("block_helpdesk_ticket",['status'=>'2']);
        }
