...
$(window).on('load', function() {
            // add your functions
  counterNumber();
            /* We do this for M4 More Menu.Should have no effect on earlier Mdl versions */
...
$(document).ready(function() {
  ...
  header_color = String(header_color);
  if (!header_color) {
    return false;
  }

  var parts = header_color.match(/^rgb\((\d+),\s*(\d+),\s*(\d+)\)$/);
  delete(parts[0]);
  ...
  $('header.header-nav.menu_style_home_one .ace-responsive-menu li a').css('color', textColour);
});
